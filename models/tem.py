...
class Threat(Base):
    ...
    threat_id = Column(Integer, primary_key=True)
...
class UndesiredAircraftState(Base):
    ...
    uas_id = Column(Integer, primary_key=True)
    event_id = Column(Integer, ForeignKey("events.event_id", ondelete="CASCADE"), nullable=False)
    state_description = Column(Text, nullable=False)
    state_type = Column(String(64), nullable=False)

    event = relationship("Event", back_populates="undesired_aircraft_states")


class Countermeasure(Base):
    __tablename__ = "countermeasures"

    countermeasure_id = Column(Integer, primary_key=True)
    event_id = Column(Integer, ForeignKey("events.event_id", ondelete="CASCADE"), nullable=False)
    countermeasure_description = Column(Text, nullable=False)
    countermeasure_type = Column(String(64), nullable=False)
    competency_involved = Column(String(32), nullable=False)
    effectiveness = Column(String(32), nullable=False)
